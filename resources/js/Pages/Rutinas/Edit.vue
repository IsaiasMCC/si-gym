<template>
  <div class="min-h-screen p-6" :style="{ color: 'var(--color-text)', backgroundColor: 'var(--color-bg)' }">
    ...
    <h1 class="mb-8 text-3xl font-bold">
      <Link class="text-indigo-400 hover:text-indigo-600" href="/inf513/grupo18sc/proyecto2/sis-gym/public/rutinas">
      Rutinas</Link>
      ...
      <form @submit.prevent="update">
        <div class="flex flex-wrap -mb-8 -mr-6 p-8">
          ...
          <text-input v-model="form.descripcion" :error="form.errors.descripcion" class="pb-8 pr-6 w-full lg:w-1/2"
            label="Descripción" />
          <text-input v-model="form.objetivo" :error="form.errors.objetivo" class="pb-8 pr-6 w-full lg:w-1/2"
            label="Objetivo*" />
          <select-input v-model="form.nivel" :error="form.errors.nivel" class="pb-8 pr-6 w-full lg:w-1/2"
            label="Nivel*">
            <option value="principiante">Principiante</option>
            <option value="intermedio">Intermedio</option>
            <option value="avanzado">Avanzado</option>
          </select-input>
          <select-input v-model="form.entrenador_id" :error="form.errors.entrenador_id"
            class="pb-8 pr-6 w-full lg:w-1/2" label="Entrenador*">
        ...
        <div class="flex items-center px-8 py-4 border-t">
          <button v-if="canAny" class="text-red-600 hover:underline" type="button" @click="destroy">Eliminar Rutina</button>
          <loading-button :loading="form.processing" class="btn-indigo ml-auto" type="submit">Actualizar
            Rutina</loading-button>
        </div>
      </form>
    </div>
  </div>
